// X-Gnarly CLI: 复用 tiktok_sign.rs 算法
mod xg {
...
fn main() {
    ...
    if args.len() < 3 { eprintln!("用法: xgnarly_cli <query> <ua> [body]"); std::process::exit(1); }
    let sig = xg::x_gnarly(&args[1], &args[2], args.get(3).map(String::as_str).unwrap_or(""), None);
    println!("{sig}");
}
